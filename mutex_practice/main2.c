...
void screenPrinter(char c) {
    pthread_mutex_lock(&mutex1); // Sekcja krytyczna - zablokowanie mutexu przed dostępem do drukarki
    printf("%c\n",c);
    // drukarka drukuje 4 znaki/s
    usleep(250*1000);
    pthread_mutex_unlock(&mutex1); // Zwolnienie mutexu po zakończeniu drukowania
}
